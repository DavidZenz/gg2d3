...
gg2d3 supports the core Cartesian geoms below plus polygon-family `geom_sf()`.
...
### Polygon-family maps with `geom_sf`

`geom_sf()` is supported for polygon-family choropleths and polygon overlays.
This example uses the `nc` shapefile bundled with `sf` and renders county
boundaries as D3 `path` marks.
...
The v1.8 `geom_sf()` support contract is intentionally narrow:

- Only `POLYGON` and `MULTIPOLYGON` geometries render.
...
- Unsupported, empty, invalid, or missing geometry rows emit
  `geom_sf layer skipped %d unsupported, empty, invalid, or missing geometries`
  and are skipped while valid polygon rows remain renderable.
- gg2d3 does not provide tile basemaps, slippy map controls, JavaScript-side
  CRS reprojection, polygon-overlap brushing, non-polygon sf rendering, or
...
2. **Unsupported geoms** — when a geom type has no D3 renderer, an in-panel
   message appears (e.g., "unsupported geom: polygon") rather than producing
