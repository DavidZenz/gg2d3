...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 54-01-01 | 01 | 0 | GEOM-01, GEOM-04 | T-54-01 | Labels/text use SVG `.text(...)`, not HTML insertion | unit/source | `rtk Rscript --vanilla -e 'pkgload::load_all(quiet=TRUE); testthat::test_file("tests/testthat/test-text-label-polish.R")'` | yes | ⬜ pending |
| 54-02-01 | 02 | 0 | GEOM-02 | T-54-02 | Subgroup metadata is either rendered by a bounded contract or not exposed as implied support | unit/source | `rtk Rscript --vanilla -e 'pkgload::load_all(quiet=TRUE); testthat::test_file("tests/testthat/test-polygon-ir.R"); testthat::test_file("tests/testthat/test-polygon-renderer.R")'` | yes | ⬜ pending |
| 54-03-01 | 03 | 0 | GEOM-03 | T-54-03 | Invalid transformed bounds do not create malformed DOM attributes | unit/source | `rtk Rscript --vanilla -e 'pkgload::load_all(quiet=TRUE); testthat::test_file("tests/testthat/test-rect-tile-ir.R"); testthat::test_file("tests/testthat/test-rect-tile-renderer.R")'` | yes | ⬜ pending |
| 54-04-01 | 04 | 1 | GEOM-01, GEOM-04 | T-54-01 | Text/label selectors and private fields remain covered by renderer contracts | source contract | `rtk Rscript --vanilla -e 'pkgload::load_all(quiet=TRUE); testthat::test_file("tests/testthat/test-renderer-wiring-contracts.R")'` | yes | ⬜ pending |
| 54-05-01 | 05 | 1 | GEOM-01, GEOM-02, GEOM-03, GEOM-04 | — | User-facing diagnostics distinguish shipped support from explicit future work | docs/source | `rg -n "geom_label|geom_polygon|subgroup|hole|hjust|vjust|angle|collision|path-following|rect|tile" vignettes/d3-drawing-diagnostics.md README.Rmd README.md` | yes | ⬜ pending |
| 54-06-01 | 06 | 2 | GEOM-01, GEOM-02, GEOM-03, GEOM-04 | T-54-01/T-54-02/T-54-03 | Browser artifacts, if used, demonstrate selectors/rows without relying on pixel thresholds | browser smoke | `rtk env NOT_CRAN=true GG2D3_BROWSER_VISUAL_SMOKE=true Rscript --vanilla -e 'pkgload::load_all(quiet=TRUE); testthat::test_file("tests/testthat/test-browser-visual-smoke.R")'` | yes | ⬜ pending |
