...
must_haves:
  truths:
    ...
    - "IR validation runs before JavaScript receives data (from Plan 01-04)"
  artifacts:
...
@.planning/phases/01-foundation-refactoring/01-04-SUMMARY.md
@inst/htmlwidgets/gg2d3.js
@inst/htmlwidgets/gg2d3.yaml
</context>
...
Open each HTML file and verify rendering. Also run `devtools::test()` to verify the R test suite still passes (including the IR validation tests from Plan 01-04).
  </action>
  <verify>
1. `devtools::test()` -- all R tests pass (including test-validate-ir.R from Plan 01-04)
...
6. Line count of gg2d3.js is under 200 lines (was 717)
  ...
  <done>
gg2d3.js is refactored to use modular dispatch. draw() is ~120-150 lines of orchestration. All inline helpers, scale creation, theme logic, and geom rendering are replaced with namespace calls to the extracted modules. gg2d3.yaml loads all scripts. The widget renders correctly. R test suite passes including IR validation.
  </done>
</task>
...
- geom-registry.js (dispatch registry + color accessors)
- 5 geom renderer files (point, line, bar, rect, text)
- Refactored gg2d3.js (orchestration only)
- IR validation (validate_ir.R) integrated into R pipeline
...
6. devtools::test() passes all R tests (including IR validation from Plan 01-04)
...
- R test suite passes completely (including IR validation tests)
