...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 34-01-01 | 01 | 1 | SFREND-02 | T-34-01 | Shared `sf_bbox` spans all accepted sf layers in the panel | unit | `Rscript --vanilla -e 'pkgload::load_all(quiet = TRUE); testthat::test_file("tests/testthat/test-sf-ir.R")'` | yes | passed |
| 34-01-02 | 01 | 1 | SFREND-02, SFREND-03 | T-34-02 | `sf_bbox` is panel-keyed and validates without Cartesian ranges | unit | `Rscript --vanilla -e 'pkgload::load_all(quiet = TRUE); testthat::test_file("tests/testthat/test-sf-ir.R")'` | yes | passed |
| 34-02-01 | 02 | 2 | SFREND-02 | T-34-03 | `sf.js` fits from shared panel bbox when available | source/unit | `Rscript --vanilla -e 'pkgload::load_all(quiet = TRUE); testthat::test_file("tests/testthat/test-sf-renderer.R")'` | yes | passed |
| 34-02-02 | 02 | 2 | SFREND-03 | T-34-04 | Faceted JS filters sf geometries with filtered data | source/unit | `Rscript --vanilla -e 'pkgload::load_all(quiet = TRUE); testthat::test_file("tests/testthat/test-sf-renderer.R")'` | yes | passed |
| 34-03-01 | 03 | 3 | SFREND-03 | T-34-05 | `facet_wrap()` sf panels fit from their own rows only | unit | `Rscript --vanilla -e 'pkgload::load_all(quiet = TRUE); testthat::test_file("tests/testthat/test-facets.R")'` | yes | passed |
| 34-03-02 | 03 | 3 | SFREND-03 | T-34-06 | `facet_grid()` preserves layout and empty panels do not use global bbox fallback | unit | `Rscript --vanilla -e 'pkgload::load_all(quiet = TRUE); testthat::test_file("tests/testthat/test-facet-grid.R")'` | yes | passed |
