...
If both `flip` and `ratio` are present (which ggplot2 does allow via `coord_flip()` being superseded by flipping aesthetics), the ratio should still apply. However, `coord_flip` and `coord_fixed` are mutually exclusive coordinate systems in ggplot2 — a plot uses one or the other. So no special interaction handling is needed.
  </action>
  <verify>
Create test script:
```r
...
7. Resize verification: Open the p1 widget in a browser, manually resize the browser window. Confirm the panel recalculates dimensions while maintaining the square aspect ratio (it should not stretch to fill the new dimensions). After resize, axis labels and title should remain at constant pixel size and stay correctly positioned.
  </verify>
